...
def create_prm_traj(map_file):
    ...
    knn_edges = np.array([(0,0)])
    rnn_edges = np.array([(0,0)])
    for idx, node in enumerate(samples):
        ...
        for target_node_idx in knn_edge_sampling:  # for each node in the sampled list
            ...
            edge = np.array([(idx, target_node_idx)])
            #### expected result - array of 10 points (x,y) along the line node -> target node
            ...
            #### expected result - array of True/False for each point in line
            if(is_edge_colliding == False):
                knn_edges = np.concatenate((knn_edges, edge))
            #### expected result - knn_edges grows by [(node,
        ...
        for target_node_idx in rnn_edge_sampling: # same as knn - but for the rnn sampling
            ...
            if (is_edge_colliding == False):
                rnn_edges = np.concatenate((rnn_edges, edge))

    knn_edges = np.delete(knn_edges, 0, 0)  # remove the first dummy element used when initialized the array
    rnn_edges = np.delete(rnn_edges, 0, 0)  # remove the first dummy element used when initialized the array
    ...
    knn_costs.pop((0,0)) # remove the first dummy element used when initialized the array

    rnn_costs = {(0,0): 0}
    for edge in rnn_edges:
        edge_cost = np.linalg.norm(samples[edge[0]] - samples[edge[1]], ord = 2)
        rnn_costs[edge[0], edge[1]] = edge_cost
    rnn_costs.pop((0,0)) # remove the first dummy element used when initialized the array
    ...
    knn_prm_graph = {'nodes': samples,
             'edges': knn_edges,
             'costs': knn_costs}
    knn_prm_graph['nodes'] = np.array(knn_prm_graph['nodes'])

    rnn_prm_graph = {'nodes': samples,
                     'edges': rnn_edges,
                     'costs': rnn_costs}
    rnn_prm_graph['nodes'] = np.array(rnn_prm_graph['nodes'])
    ...
    prm_traj = np.concatenate((prm_traj, rnn_astar.a_star(mid_points[0], mid_points[1])) , axis=0)
